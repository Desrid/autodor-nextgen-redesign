...
export function RoadStatistics() {
  ...
  const title = road.label.startsWith(road.shortLabel)
    ...
    : road.label;
  const selectRoad = (offset: number) =>
    setActiveIndex((activeIndex + offset + ROADS.length) % ROADS.length);

  return (
    <>
      <div className="statistics-header">
        <div className="section-heading">
          <h2 id="statistics-title">Статистика</h2>
        </div>
        <div
          className="statistics-controls"
          role="group"
          aria-label="Навигация по трассам"
        >
          <button
            type="button"
      ...
      <figure
        className="tariff-statistics"
        data-testid="tariff-statistics"
      ...
      >
        <figcaption className="tariff-statistics__intro">
          ...
          <dl
            className="tariff-statistics__facts"
            aria-label={`Сводка трассы ${road.shortLabel}`}
          >
            <div>
              <dt>Протяжённость</dt>
              <dd>{road.fact.extent}</dd>
            </div>
            <div>
              <dt>Скорость</dt>
        ...
        <div className="tariff-statistics__data">
          <div
            ...
            role="group"
            aria-label={`Базовые тарифы по категориям транспорта для ${road.shortLabel}`}
          >
            <div className="tariff-chart__heading">
            ...
            {VEHICLE_CATEGORY_TARIFFS.map((item) => (
              <div className="tariff-chart__row" key={item.category}>
                ...
                <span className="tariff-chart__track" aria-hidden="true">
                  <span
          ...
          <p className="tariff-statistics__verified">
            Данные сверены{" "}
            <time dateTime={road.factSource.verifiedAt}>
              {road.factSource.verifiedAt.split("-").reverse().join(".")}
            </time>
          </p>
        </div>
        ...
        <div className="tariff-sources">
          <p>
            Данные по трассе опубликованы на официальном сайте Государственной компании
            «Автодор».
          </p>
          <a href={road.detailsUrl}>Официальная страница {road.shortLabel}</a>
        </div>
      </figure>
    </>
  );

    ...
    <figure className="statistics-dashboard" data-testid="statistics-dashboard">
      ...
      >
        <div className="statistics-panel__header">
          <p>Строительство автомобильных дорог</p>
          <h3 id="statistics-years-title">Построено по годам</h3>
        </div>

        <div className="statistics-year-highlight" aria-hidden="true">
          <strong>{formatDistanceKm(YEARLY_STATISTICS[0].distanceKm)}</strong>
          <span>км построено в 2025 году</span>
        </div>
      ...
      >
        ...
        >
          {[...YEARLY_STATISTICS].reverse().map((item) => {
            ...
            return (
              <div className="statistics-bars__item" key={item.year}>
                <button
                  ...
                  aria-label={`${item.year} год, ${formatDistanceKm(item.distanceKm)} километра`}
                >
      ...
      <p className="statistics-source">
        Источник:{" "}
        <a href={STATISTICS_SOURCE.url}>
          {STATISTICS_SOURCE.label}, диапазоны {STATISTICS_SOURCE.yearlyRange} и{" "}
          {STATISTICS_SOURCE.operationsRange}
        </a>
        . Итог и процентные доли рассчитаны из исходных значений.
      </p>
    </figure>
  );
}
